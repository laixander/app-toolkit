...
<template>
    <UDrawer v-model:open="isOpen" direction="right" inset title="Application Logs"
        description="View real-time application events and status." :ui="{ container: 'sm:py-6 sm:pr-8' }" class="min-w-md">
        <template #header>
        ...
        <template #footer>
            <div class="w-full mt-4">
                <UButton label="Clear Logs" icon="i-lucide-trash-2" color="error" variant="soft" size="md"
                    @click="clearLogs" :disabled="!logs.length" block />
            </div>
        </template>
    </UDrawer>
</template>
